...
export class LatexController {
  ...
  async compileLatexToPdf(
    @Body() body: { latexContent: string },
    @Request() req
  ) {
    try {
      // Validate LaTeX content
      const validation = this.latexService.validateLatexContent(
        body.latexContent
      );
  ...
  async validateLatex(@Body() body: { latexContent: string }) {
    try {
      ...
      const metadata = this.latexService.extractLatexMetadata(
        body.latexContent
      );
  ...
  async compileLatexWithDocker(
    @Body() body: { latexContent: string },
    @Request() _req
  ) {
    try {
      const pdfBuffer = await this.dockerLatexService.compileLatexToPdf(
        body.latexContent
      );
      return {
        success: true,
  ...
  async testDockerCompilation(@Request() _req) {
    try {
      ...
      return {
        success,
        message: success
          ? 'Docker LaTeX compilation test passed'
          : 'Docker LaTeX compilation test failed',
      };
    } catch (error) {
      return {
        success: false,
