...
export function CoverLetter() {
  ...
  const handleDownloadTXT = () => {
    if (jobHistory) {
      ...
      element.download = `${jobHistory.jobTitle}-cover-letter.txt`;
  ...
  const handleCopy = async () => {
    if (jobHistory) {
      await navigator.clipboard.writeText(jobHistory.coverLetter);
  ...
  if (loading) {
    return (
      <Box
        ...
        p={6}
      >
        <Text>Loading cover letter...</Text>
      </Box>
    );
  }
  ...
  if (!jobHistory) {
    return (
      <Box
        ...
        p={6}
      >
        <Text>No cover letter data found.</Text>
      </Box>
    );
  }
